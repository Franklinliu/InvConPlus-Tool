...
contract Kek_Inu is ERC20Interface, SafeMath {
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        ...
        VeriSol.Ensures((!(VeriSol.Old(balances[msg.sender]) == tokens)) || (tokens >= balances[msg.sender]));
        ...
        VeriSol.Ensures((!(VeriSol.Old(balances[msg.sender]) > VeriSol.Old(balances[to]))) || ((VeriSol.Old(balances[msg.sender]) > balances[msg.sender]) || (VeriSol.Old(balances[msg.sender]) != balances[to])));
        VeriSol.Ensures((!((VeriSol.Old(balances[msg.sender]) > VeriSol.Old(balances[to])) && (VeriSol.Old(balances[to]) != 0))) || (balances[to] != tokens));
        VeriSol.Ensures((!((VeriSol.Old(balances[msg.sender]) != VeriSol.Old(balances[to])) && (msg.sender == to))) || (balances[msg.sender] == 445000000000000000000000000000));
        VeriSol.Ensures((!((VeriSol.Old(balances[msg.sender]) > tokens) && (tokens == VeriSol.Old(VeriSol.SumMapping(balances))))) || (VeriSol.Old(balances[msg.sender]) != VeriSol.SumMapping(balances)));
