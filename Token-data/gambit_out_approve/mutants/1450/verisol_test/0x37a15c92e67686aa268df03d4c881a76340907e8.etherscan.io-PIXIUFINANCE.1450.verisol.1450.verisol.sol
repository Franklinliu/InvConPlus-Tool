...
contract PIXIUFINANCE is ERC20Interface, SafeMath {
    ...
    function transfer(address to, uint tokens) public returns (bool success) {
        ...
        VeriSol.Ensures((!(tokens > 0)) || ((tokens != VeriSol.SumMapping(balances)) || (VeriSol.SumMapping(balances) > 0)));
        VeriSol.Ensures((!((VeriSol.Old(balances[msg.sender]) > tokens) && (tokens >= VeriSol.Old(balances[to])))) || ((balances[msg.sender] + tokens) == VeriSol.Old(balances[msg.sender])));
    ...
    function contractInvariant() private view {
        VeriSol.ContractInvariant(VeriSol.SumMapping(balances) == 10000000);
        VeriSol.ContractInvariant(decimals == 0);
        VeriSol.ContractInvariant(_totalSupply == 10000000);
