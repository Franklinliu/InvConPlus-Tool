...
contract HERO is TokenDetailed, ERC20Burnable, Stoppable {
    ...
    constructor(string memory name, string memory symbol, uint256 totalSupply, uint8 decimals) public TokenDetailed(name,symbol,decimals) {
        _mint(owner(), totalSupply * (uint(decimals) ** 10));
    }
    ...
    ///  @return Boolean
    function setLock(address account, uint256 releaseTime, uint256 amount) public onlyOwner() {
    ...
    function _isLocked(address account, uint256 amount) internal view returns (bool) {
        return ((_lockAmounts[account] != 0) && (_lockTimes[account] > block.timestamp)) && ((balanceOf(account) <= _lockAmounts[account]) || (balanceOf(account).sub(_lockAmounts[account]) < amount));
    }
    ...
    ///  @return Boolean 
    function transfer(address recipient, uint256 amount) public enabled() returns (bool) {
        ...
        VeriSol.Ensures(VeriSol.Old(stopped) == false);
        VeriSol.Ensures(stopped == false);
        VeriSol.Ensures(VeriSol.Old(VeriSol.SumMapping(_lockTimes)) == VeriSol.SumMapping(_lockTimes));
        ...
        VeriSol.Ensures(VeriSol.Old(VeriSol.SumMapping(_lockAmounts)) == VeriSol.SumMapping(_lockAmounts));
        //End of invariants
    ...
    ///  @param amount Token transfer amount.
    ///  @return Boolean 
    function transferFrom(address sender, address recipient, uint256 amount) public enabled() returns (bool) {
