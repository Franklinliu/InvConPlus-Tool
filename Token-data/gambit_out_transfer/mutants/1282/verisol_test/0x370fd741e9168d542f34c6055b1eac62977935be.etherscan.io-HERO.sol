...
contract HERO is TokenDetailed, ERC20Burnable, Stoppable {
    ...
    function _isLocked(address account, uint256 amount) internal view returns (bool) {
        return _lockAmounts[account] != 0 && 
            ...
            (
                /// SwapArgumentsOperatorMutation(`balanceOf(account) <= _lockAmounts[account]` |==> `_lockAmounts[account] <= balanceOf(account)`) of: `balanceOf(account) <= _lockAmounts[account] ||`
                _lockAmounts[account] <= balanceOf(account) ||
