...
contract HERO is TokenDetailed, ERC20Burnable, Stoppable {
    ...
    function _isLocked(address account, uint256 amount) internal view returns (bool) {
        return ((_lockAmounts[account] != 0) && (_lockTimes[account] > block.timestamp)) && ((_lockAmounts[account] <= balanceOf(account)) || (balanceOf(account).sub(_lockAmounts[account]) < amount));
    }
