...
namespace SolToBoogie
{
    ...
    public class HarnessGenerator
    {
        ...
        public void Generate()
        {
            if (!context.TranslateFlags.NoBoogieHarness)
            {
                foreach (ContractDefinition contract in context.ContractDefinitions)
                {
                    ...
                    (Dictionary<int, BoogieExpr> houdiniVarMap, int derivedHoudiniCandidateCount) =
                        HoudiniHelper.GenerateHoudiniVarMapping(contract, context, contractInvariants);
                    
                    GenerateHoudiniVarsForContract(contract, houdiniVarMap, derivedHoudiniCandidateCount);
                    GenerateBoogieHarnessForContract(contract, houdiniVarMap, derivedHoudiniCandidateCount);
                }
            }
        ...
        private void GenerateHoudiniVarsForFunc(FunctionDefinition function, Dictionary<int, BoogieExpr> houdiniVarMap)
        {
            foreach (int id in houdiniVarMap.Keys)
            {
                string varName = GetHoudiniVarName(id, function);
                ...
                // Add the declaration to function's implementation
                context.Program.AddDeclaration(houdiniVar);
            }
        }

        private void GenerateHoudiniVarsForContract(ContractDefinition contract, Dictionary<int, BoogieExpr> houdiniVarMap, int derivedHoudiniCandidateCount)
        {
            foreach (int id in houdiniVarMap.Keys)
            {
                string varName = GetHoudiniVarName(id, contract, derivedHoudiniCandidateCount);
                BoogieConstant houdiniVar = new BoogieConstant(new BoogieTypedIdent(varName, BoogieType.Bool));
                houdiniVar.Attributes = new List<BoogieAttribute>()
                {
                    new BoogieAttribute("existential", true)
                ...
                context.Program.AddDeclaration(houdiniVar);
                // Console.WriteLine($"... Added Declaration {varName}");
            }
        }
        ...
        private void GenerateBoogieHarnessForContract(ContractDefinition contract, Dictionary<int, BoogieExpr> houdiniVarMap, int derivedHoudiniCandidateCount)
        {
            ...
            harnessBody.AddStatement(GenerateWhileLoop(contract, houdiniVarMap, localVars, derivedHoudiniCandidateCount));
        ...
        private BoogieWhileCmd GenerateWhileLoop(ContractDefinition contract, Dictionary<int, BoogieExpr> houdiniVarMap, List<BoogieVariable> localVars, int derivedHoudiniCandidateCount)
        {
            ...
            foreach (int id in houdiniVarMap.Keys)
            {
                BoogieIdentifierExpr houdiniVar = new BoogieIdentifierExpr(GetHoudiniVarName(id, contract, derivedHoudiniCandidateCount));
            ...
            //     {
            //         BoogieIdentifierExpr houdiniVar = new BoogieIdentifierExpr(GetHoudiniVarName(0, contract));
            //         BoogieExpr invExpr = new BoogieBinaryOperation(BoogieBinaryOperation.Opcode.IMP, houdiniVar, inv);
            //         BoogieLoopInvCmd invCmd = new BoogieLoopInvCmd(invExpr);
            //         candidateInvs.Add(invCmd);
            //     }
        ...
        private string GetHoudiniVarName(int id, ContractDefinition contract, int derivedHoudiniCandidateCount)
        {
            if (id<=derivedHoudiniCandidateCount)
                return "HoudiniB" + id.ToString() + "_" + contract.Name;
            else
                return "HoudiniC" + id.ToString() + "_" + contract.Name;
        }

        private string GetHoudiniVarName(int id, FunctionDefinition func)
        {
            return "HoudiniF" + id.ToString() + "_" + func.Name;
        }
