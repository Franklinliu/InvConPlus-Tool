...
namespace SolToBoogie
{
    ...
    public class ProcedureTranslator : BasicASTVisitor
    {
        ...
        public override bool Visit(FunctionDefinition node)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    BoogieStmtList procBodyWoRequires, procBodyWoEnsures, procBodyWoModifies, procBodyWoAssumesBegin, procBodyWoAssumesEnd;
                    ...
                    var assumesBegin = ExtractSpecifications("AssumesBeginningOfFunction_VeriSol", procBodyWoModifies, out procBodyWoAssumesBegin);
                    var assumesEnd = ExtractSpecifications("AssumesEndingOfFunction_VeriSol", procBodyWoAssumesBegin, out procBodyWoAssumesEnd);

                    // insert the assumption statements to the begining of the procedure body
                    for (int i = assumesBegin.Count - 1; i >= 0; i--)
                    {
                        procBodyWoAssumesEnd.insertStatementAtBeginning(new BoogieAssumeCmd(assumesBegin[i]));
                    }

                    // insert the assumption statements to the end of the procedure body
                    for (int i = 0; i < assumesEnd.Count; i++)
                    {
                        procBodyWoAssumesEnd.AddStatement(new BoogieAssumeCmd(assumesEnd[i]));
                    }

                    // Generate Houdini variables names for the current procedure
                    List<BoogieConstant> houdinivars = new List<BoogieConstant>();
                    for (int i = 0; i < (preconditions.Count + postconditions.Count); i++)
                    {
                        BoogieConstant houdinivar = new BoogieConstant(new BoogieTypedIdent($"HoudiniF{context.GetAndIncreaseHoudiniVarCount()}_{procName}", BoogieType.Bool));
                        houdinivar.Attributes = new List<BoogieAttribute>()
                        {
                            new BoogieAttribute("existential", true)
                        };
                        houdinivars.Add(houdinivar);
                    }

                    procedure.AddHoudiniVariables(houdinivars);
                    ...
                    {
                        BoogieImplementation impelementation = new BoogieImplementation(procName, inParams, outParams, localVars, procBodyWoAssumesEnd);
        ...
        private BoogieExpr TranslateVeriSolCodeContractFuncCall(FunctionCall node)
        {
            ...
            // HACK for SumIntInt
            if (verisolFunc.Equals("_SumMappingIntInt_VeriSol"))
            {
                //has to be M_ref_int[mapp[this]] instead of mapp[this]
                var mapName = MapArrayHelper.GetMemoryMapName(BoogieType.Int, BoogieType.Int);
                boogieExprs[0] = new BoogieMapSelect(new BoogieIdentifierExpr(mapName), boogieExprs[0]);
            }
                ...
                if (member.Expression is Identifier ident)
                {
                    if (ident.Name.Equals("VeriSol"))
                    {
                        ...
                        if (member.MemberName.Equals("Invariant") ||
                            ...
                            member.MemberName.Equals("AssumesBeginningOfFunction") ||
                            member.MemberName.Equals("AssumesEndingOfFunction") 
                            )
                            return false;
                        else
                            return true;
            ...
            if (node.Expression is MemberAccess member)
            {
                if (member.Expression is Identifier ident)
                {
                    if (ident.Name.Equals("VeriSol"))
                    {
                        ...
                        if (member.MemberName.Equals("SumMappingIntInt"))
                            return "_SumMappingIntInt_VeriSol";
