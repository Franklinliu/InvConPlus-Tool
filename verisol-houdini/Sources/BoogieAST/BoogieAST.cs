...
namespace BoogieAST
{
    ...
    public class BoogieProcedure : BoogieDeclWithFormals
    {
        ...
        List<BoogieConstant> HoudiniVars;
        public BoogieProcedure(string name, List<BoogieVariable> inParams, List<BoogieVariable> outParams, List<BoogieAttribute> attributes = null, List<BoogieGlobalVariable> modSet = null, List<BoogieExpr> pre = null, List<BoogieExpr> post = null, List<BoogieConstant> houdinivars = null)
        {
            this.Name = name;
            ...
            this.HoudiniVars = houdinivars;
        }

        public override string ToString()
        {
            StringBuilder builder = new StringBuilder();
            // Add houdnin variables declaration
            if (HoudiniVars != null)
            {
                foreach (BoogieConstant houdinivar in HoudiniVars)
                {
                    builder.Append(houdinivar);
                }
            }
            ...
            if (preConditions != null)
            {
                ...
                {
                    if (HoudiniVars != null && HoudiniVars.Count > 0)
                    {

                        builder.AppendLine($"requires {HoudiniVars[0].Name} ==> ({e.ToString()});");
                        HoudiniVars.RemoveAt(0);
                    }
                    else
                    {
            ...
            if (postConditions != null)
            {
                foreach (var e in postConditions)
                {
                    if (HoudiniVars != null && HoudiniVars.Count > 0)
                    {
                        ...
                        builder.AppendLine($"ensures {HoudiniVars[0].Name} ==> ({e.ToString()});");
                        HoudiniVars.RemoveAt(0);
                    }
                    else
                    {
        ...
        public void AddHoudiniVariables(List<BoogieConstant> houds)
        {
            if (HoudiniVars == null)
                HoudiniVars = new List<BoogieConstant>();
            HoudiniVars.AddRange(houds);
        }
    }
    ...
    public class BoogieStmtList : BoogieASTNode
    {
        ...
        public void insertStatementAtBeginning(BoogieCmd cmd)
        {
            Debug.Assert(BigBlocks.Count == 1);
            BigBlocks[0].insertStatementAtBeginning(cmd);
        }
    ...
    public class BoogieBigBlock : BoogieASTNode
    {
        ...
        public void insertStatementAtBeginning(BoogieCmd statement)
        {
