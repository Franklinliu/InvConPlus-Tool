...
import traceback
import math 
import invconplus.daikon.Config as config
import invconplus.daikon.Daikon as daikon
from alive_progress import alive_bar
from .Daikon import *
import invconplus.vmtracesimulator.main as vmtrace
...
def Type_init(self, astId, contract, label, offset, slot, type):
    ...
    try:
        if self.encoding=="dynamic_array":
            self.firstElementSlot = toInt(Web3.solidity_keccak(["uint256"], [self.slot]))
            self.value = 0
            self.elements = list() 
            ...
            if self.isInplace() and not self.isFixedArray():
                ...
                if basecls.isInplace():
                    if not basecls.isStruct():
                        ...
                        structdef = ContractStorageMonitor(basecls.members,  Web3.solidity_keccak(["uint256"], [self.slot]))
                        for storage in structdef.storages:
                            fieldname = storage.getLabel()
                            if storage.isInplace():
                                if not storage.isStruct():
                ...
                if valuecls.isInplace() and not valuecls.isFixedArray():
                    if not valuecls.isStruct():
                        ...
                        structdef = ContractStorageMonitor(valuecls.members,  Web3.solidity_keccak(["uint256"], [self.slot]))
                        for storage in structdef.storages:
                            fieldname = storage.getLabel()
                            if storage.isInplace():
                                if not storage.isStruct():
        ...
        if self.slot <= int(slot, 16) and int(slot, 16) <= self.slot + int((int(self.numOfBytes) + int(self.offset) -1) / 32):
            if self.isEnum():
                ...
                if self.basecls is not None:
                    ...
                    value = "0x"+ value[2:][64-2*(self.offset+self.numOfBytes):64-2*self.offset].replace("0x", "")
                    if self.getType().find("int")!=-1:
                        self.value = int(value, 16)
...
class ContractStorageMonitor:
        ...
        if value.find("-")!=-1:
            ...
            if slot in self.availableslots:
                if self.availableslots[slot] != oldval.strip():
                    print(f"Warning!!! the recorded value of {slot} is inconsistent. recorded: {self.availableslots[slot]} vs actual: {oldval}")
            # else:
    ...
    def txStateTransition(self, slot_statechanges, funcName, additionalKeys, msg_sender, msg_value, args, showflag=False, txHash=None):
        ...
        missing_slots = list() 
        for slot in slot_statechanges:
                value_from = slot_statechanges[slot]["*"]["from"]
                value_to = slot_statechanges[slot]["*"]["to"]
                additionalKeys.update(self.getAllInplaceValues())
                if not self.readStateChange(slot, value_from + "-" + value_to, additionalKeys=additionalKeys):
                    missing_slots.append([slot, value_from + "-" + value_to])
        
        if len(missing_slots) > 0:
            constants = vmtrace.fetchSlotCalculationConstants(None, txHash)
            for item in missing_slots:
              slot, from_to = item 
              if not self.readStateChange(slot,from_to, additionalKeys=constants):
                    print("{0} not found which has value {1}".format(slot, from_to))
...
def isString(v_type):
    return  v_type.find("address")!=-1 or v_type.find("bytes")!=-1 or v_type.find("string")!=-1 or  v_type.find("contract")!=-1
def isBool(v_type):
...
def translate(value, param):
    if re.match("^int[0-9]*$", param["type"]) is not None:
        return [int(value)]
    elif re.match("^uint[0-9]*$", param["type"]) is not None:
        return [int(value)]
    elif re.match("bytes[0-9]*", param["type"]) is not None:
        return [value]
    elif re.match("address", param["type"]) is not None:
        return [value]
    elif re.match("bool", param["type"]) is not None:
        return [value]
    elif re.match("string", param["type"]) is not None:
        return [value]
    elif re.match("^(\w+)\[.*\]$", param["type"]) is not None:
        m = re.match("^(\w+)\[.*\]$", param["type"])
        result = list()
        for _item in value:
            result.append(translate(_item, m.group(1)))
        return result 
    else:
        return value 

class Contract(ContractStorageMonitor):
    def __init__(self, address, workdir, contractName, storageLayoutJson, input_abi, transactions):
       
        self.address = address 
        self.workdir = workdir
        layoutjson = storageLayoutJson
        types = layoutjson["types"]
        storage = layoutjson["storage"]

        super().__init__(typeJson=types, storageJson = storage)

        self.tx_abi = input_abi
    ...
    def readTransactionFunctionInputVariables(self, tx, nonce):
        ...
        if "articulatedTrace" not in tx:
            return [], [], [], [], [], None 

        assert "articulatedTrace" in tx, "no decoded parameters" 
       

        tx_hash = tx["transactionHash"]
        
        decoded = tx["articulatedTrace"]
        
        funcName = decoded["name"]
        if "inputs" in decoded:
            args = decoded["inputs"]
        else:
            args = dict()
        if isinstance(args, dict):
            functionABI = list(filter(lambda function: function["type"] == "function"  and function["name"]==decoded["name"], self.tx_abi))[0]
            keys = args.keys()
            count = 0
            for _var in keys:
                value = args[_var]
                param = functionABI["inputs"][count]
                count += 1
                self.envs.update(translate(value, param))
        ...
        try:
            functionABI = list(filter(lambda function: function["type"] == "function"  and function["name"]==funcName, self.tx_abi))[0]
            
            enter = ["""{0}.{1}:::ENTER
this_invocation_nonce
{2}""".format(self.contractName, _get_full_function_name(funcName, functionABI["inputs"]), nonce)]
...
{2}""".format(self.contractName, _get_full_function_name(funcName, functionABI["inputs"]), nonce)]
            ...
            if len(args)>0:
                ...
                for var in variables:
                    param_name = functionABI["inputs"][count]["name"]
                    var_type = functionABI["inputs"][count]["type"]
                    count += 1
                    ...
                    if not isArray(var_type):
                        ...
                        v = """{0}  
{1}
1""".format(param_name, "\"{0}\"".format(args[var]) if isString(var_type) else str(int(args[var])) if not isBool(var_type) else 1 if args[var] else 0) 
                        dtrace.append(v)
                    else:
                        _vars = args[var]
                        ...
                        v = """{0}  
{1}
1""".format(param_name, 123456) 
                        dtrace.append(v)
                        v = """{0}[..]  
[{1}]
1""".format(param_name, " ".join(_vars)) 
                        dtrace.append(v)
        
            return enter, exit1, exit2, dtrace, args, funcName
        except:
            # fallback function
            traceback.print_exc()
    ...
    def readTransactionEnvironmentVariables(self, tx):
            dtrace = []
            v = """msg.sender
"{0}"
1""".format(tx["action"]["from"].lower()) 
            dtrace.append(v)
            v = """msg.value
{0}
1""".format(tx["action"]["value"]) 
            dtrace.append(v)
            v = """block.timestamp
{0}
1""".format(tx["timestamp"]) 
            ...
            self.envs.add(tx["action"]["from"].lower())
            return tx["action"]["from"].lower(), tx["action"]["value"], dtrace
    ...
    def readAllTxs(self):
        ...
        with alive_bar(min(LIMIT, len(self.transactions)), force_tty=True) as bar:
            for count in range(min(LIMIT, len(self.transactions))):
                self.tx_no += 1
                tx: list = self.transactions[count]
                self.envs = set() 
                tx_hash = tx[0]["transactionHash"]
                ...
                funcCall_items = list(filter(lambda item: item["action"]["to"].lower() == self.address.lower(), tx))

                funcCall_items = list(filter(lambda item: "articulatedTrace" in item and item["articulatedTrace"]["name"] != "balanceOf", funcCall_items))

                if len(funcCall_items) == 0:
                    bar()
                    continue

                msg_sender, msg_value, dtraceEnvs = self.readTransactionEnvironmentVariables(tx=funcCall_items[0])
            
                enter,exit1, exit2 = [], [], []        

                if msg_sender is not None and dtraceEnvs is not None:
                    enter, exit1, exit2, dtraceParameters, args, funcName = self.readTransactionFunctionInputVariables(funcCall_items[0], nounce)
                ...
                stateDiff = tx[0]["stateDiff"]

                if self.address in stateDiff and "storage" in stateDiff[self.address]:
                    slot_statechanges = stateDiff[self.address]["storage"]
                else:
                    slot_statechanges =  list()
    ...
    def Daikon(self, address, full_data_trace):
        self.address = address 
